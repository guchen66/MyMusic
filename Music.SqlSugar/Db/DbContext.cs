...
namespace Music.SqlSugar.Db
{
    public class DbContext<T> where T : class, new()
    {
        ...
        //注意：不能写成静态的
        public SqlSugarClient Context;//用来处理事务多表查询和复杂的操作
        public SimpleClient<T> CurrentDb { get { return new SimpleClient<T>(Context); } }//用来操作当前表的数据

        public SimpleClient<MusicSourceInfo> SourceDb { get { return new SimpleClient<MusicSourceInfo>(Context); } }//用来处理MusicSourceInfo表的常用操作
