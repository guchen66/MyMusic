...
namespace Music.System.Services.MainSign.MyFavorSign
{
    ...
    public class FavorService : IFavorService
    {
        ...
        public FavorService(IHttpClientService httpClientService)
        {
            _httpClientService = httpClientService;
        }

        private List<MusicInfoDto> HongKongDataList;

        public async Task<List<MusicInfoDto>> GetHongKongListAsync()
        {
            HongKongDataList = new List<MusicInfoDto>();
            ...
            if (ping)
            {
                ...
                try
                {
                    var html = await _httpClientService.GetStringDirectAsync(path);
                    ...
                    foreach (var ulNode in contentDiv)
                    {
                        ...
                        if (songNodes != null)
                        {
                            foreach (var songNode in songNodes)
                            {
                                ...
                                honKongData.MusicType = MusicType.Favor;
        ...
        {
            MusicInfoDto musicDto = HongKongDataList.Where(x => x.MusicName == musicName).FirstOrDefault();
